 ...
 * </ul>
 *
 * Note: mark as abstract so that the Junit4 runner doesn't
 * try to run this.
 */
abstract class JSR166TestCase extends TestCase {
   ...
   static void delay(long millis) throws InterruptedException {
      ...
      long millisLocal = millis;
      long ns = millisLocal * 1000 * 1000;
      for (;;) {
         if (millisLocal > 0L)
            Thread.sleep(millisLocal);
         else // too short to sleep
            Thread.yield();
         long d = ns - (System.nanoTime() - startTime);
         if (d > 0L)
            millisLocal = d / (1000 * 1000);
         else
            break;
      }
   }
   ...
   public Callable<String> latchAwaitingStringTask(final CountDownLatch latch) {
      return new CheckedCallable<String>() {
         ...
         protected String realCall() {
            try {
               latch.await();
            } catch (InterruptedException quittingTime) {/* Ignore */}
            return TEST_STRING;
         }};
   }
   ...
   public class SmallPossiblyInterruptedRunnable extends CheckedRunnable {
      ...
      protected void realRun() {
         try {
            delay(SMALL_DELAY_MS);
         } catch (InterruptedException ok) {/* Ignore */}
      }
   }
...
//   }
   ...
   public static class NoOpREHandler implements RejectedExecutionHandler {
      ...
      public void rejectedExecution(Runnable r,
                                    ThreadPoolExecutor executor) {/* Empty */}
   }
